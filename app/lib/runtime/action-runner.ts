...
export class ActionRunner {
    ...
    try {
      switch (action.type) {
        ...
        case 'start': {
          ...
          const { ensureProjectReady } = await import('./ensure-project-ready');
          await ensureProjectReady(webcontainer);
  ...
  async #runFileAction(action: ActionState) {
    ...
    try {
      const webcontainer = await this.#webcontainer;
      let relativePath = action.filePath.replace(/^\//, '');
      if (relativePath.startsWith(WORK_DIR + '/') || relativePath === WORK_DIR) {
        relativePath = relativePath.slice(WORK_DIR.length).replace(/^\//, '') || '';
      }
      ...
      try {
        ...
        if (normalizedPath.startsWith('src/') && /\.(jsx?|tsx?)$/.test(normalizedPath)) {
          const { ensureReactComponentStubs } = await import('./ensure-project-ready');
