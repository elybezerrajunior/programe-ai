...
export function EventLogsTab() {
  ...
  const exportAsJSON = () => {
    try {
      ...
      a.href = url;
      a.download = `programe-event-logs-${new Date().toISOString()}.json`;
  ...
  const exportAsCSV = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `programe-event-logs-${new Date().toISOString()}.csv`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const exportAsPDF = () => {
    try {
      ...
      // Add subtitle with programe.ai
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text('programe.ai - AI Development Platform', margin, 45);
      yPos = 70;
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        // Add footer text
        doc.text('Generated by programe.ai', margin, doc.internal.pageSize.getHeight() - 10);
      ...
      // Save the PDF
      doc.save(`programe-event-logs-${new Date().toISOString()}.pdf`);
  ...
  const exportAsText = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `programe-event-logs-${new Date().toISOString()}.txt`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
              ...
              {exportFormats.map((format) => (
                <button
                  ...
                  className={classNames(
                    ...
                    'bg-white dark:bg-[#050E0D]',
                    'border border-[#E5E5E5] dark:border-[#252625]',
                    'hover:bg-purple-50 dark:hover:bg-[#1a1a1a]',
                    'hover:border-purple-200 dark:hover:border-accent-900/30',
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-xs text-programe-elements-textSecondary mt-0.5">
        ...
        <DropdownMenu.Root open={showLevelFilter} onOpenChange={setShowLevelFilter}>
          ...
          <DropdownMenu.Portal>
            <DropdownMenu.Content
              className="min-w-[200px] bg-white dark:bg-[#050E0D] rounded-lg shadow-lg py-1 z-[250] animate-in fade-in-0 zoom-in-95 border border-[#E5E5E5] dark:border-[#252625]"
              sideOffset={5}
              align="start"
              side="bottom"
            >
