...
A partir desta versão, é possível configurar o provedor de LLM e o modelo exclusivamente via variáveis de ambiente, desabilitando a seleção manual na interface.

#### Variáveis de Ambiente

Para configurar o provedor e modelo via variáveis de ambiente, defina as seguintes variáveis:
...
Quando as variáveis `BOLT_LLM_PROVIDER` e `BOLT_LLM_MODEL` estão configuradas:

- ✅ O seletor de provedor e modelo **não é exibido** na interface
- ✅ O provedor e modelo são lidos automaticamente das variáveis de ambiente
- ✅ Um indicador visual discreto mostra "Configurado via variáveis de ambiente"
- ✅ As configurações não podem ser alteradas pela interface

Quando as variáveis **não estão configuradas**:

- ✅ O comportamento padrão é mantido (seleção manual disponível)
- ✅ O usuário pode escolher provedor e modelo através da interface
- ✅ As seleções são persistidas em cookies

#### Validação

O sistema valida automaticamente:

- Se o provedor especificado existe na lista de provedores disponíveis
- Se o modelo especificado é válido
- Retorna mensagens de erro descritivas se a validação falhar
...
**Erro: "Provider 'X' não encontrado"**
- Verifique se o nome do provedor está correto (case-sensitive)
- Consulte a lista de provedores disponíveis na interface
...
- Para Cloudflare Workers, certifique-se de que as variáveis estão configuradas no dashboard do Cloudflare
- Para desenvolvimento local, verifique o arquivo `.env` ou `.env.local`
